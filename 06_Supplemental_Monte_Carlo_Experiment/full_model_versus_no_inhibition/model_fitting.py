...
def run_trial(theta):
    ...
    if exists('fit_res/'+str(seed).zfill(8)+'_zero_order.npy'):
        return None
    ...
    x_hat_list_zero_order = list()
    x_hat_list_first_order = list()
    for i, init_dce in enumerate(init_dce_list):
        ...
        de_bounds = np.column_stack((x_true[[0,1,6,7]]/10, x_true[[0,1,6,7]]*10))

        de_res_zero_order = differential_evolution(
            obj_fun_zero_order,
            bounds=de_bounds,
            disp=False,
            popsize=15,
        ...
        x_hat_list_zero_order.append(de_res_zero_order.x)

        de_res_first_order = differential_evolution(
            obj_fun_first_order,
            bounds=de_bounds,
            disp=False,
            popsize=15,
        ...
        x_hat_list_first_order.append(de_res_first_order.x)

    filename = 'fit_res/'+str(seed).zfill(8)

    np.save(filename+'_zero_order', np.stack(x_hat_list_zero_order))
    np.save(filename+'_first_order', np.stack(x_hat_list_first_order))
