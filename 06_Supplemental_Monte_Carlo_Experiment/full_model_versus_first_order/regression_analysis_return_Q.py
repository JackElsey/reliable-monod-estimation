...
def gen_syn_data(theta, t95, init_dce, seed=1234):
...
def gen_flat_model_and_data(theta, seed, init_dce):
    t95 = calc_t95(theta, init_dce)

    gc_meas, qpcr_meas, gc_stdev, qpcr_stdev = gen_syn_data(theta, t95, init_dce, seed=seed)
...
def load_theta_hats(theta):
    ...
    fit_res_arr = np.load('exp2_alpha/fit_res/'+str(seed)+'_inhib.npy')
    return fit_res_arr[3:6]*scale
...
#             mrd_model, x_data, y_data, meas_stdev= gen_flat_model_and_data(theta, (seed+i+1), init_dce_list[i])
...
def calc_Q(theta):
    try:
        ...
        for i, theta_hat in enumerate(theta_hats):
            mrd_model, x_data, y_data, meas_stdev= gen_flat_model_and_data(theta, (seed+i+1), init_dce_list[i])
            Q = analyze_regression(mrd_model, x_data, y_data, meas_stdev, theta_hat, theta, conf_level=0.90)
            np.save('Q/'+str(seed)+'_'+str(i)+'Q', Q)
    except:
        pass
