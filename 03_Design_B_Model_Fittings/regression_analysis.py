...
def gen_flat_model_and_data(theta, seed):
    ...
    def mrd_model(x, theta):
        ...
        gc_mod_flat = np.stack([y[1:,:3] for y in y_list]).flatten()
        qpcr_mod_flat = np.stack([y[4::4, 3] for y in y_list]).flatten()
...
def analyze_regression(model, x_data, y_data, meas_stdev, theta_hat, theta_true, normalize=True, Q_file='none', conf_level=0.90):
    ...
    rmoe = (hi_marg_thres - lo_marg_thres)/theta_hat_mant/2
