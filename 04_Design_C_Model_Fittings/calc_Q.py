...
def gen_syn_data(theta, t95_list, seed=1234):
    ...
    for i in range(5):
        y = y_list[i]
        gc_meas_means = np.column_stack([y[:,:3]]*3)
        gc_meas = st.norm.rvs(loc=gc_meas_means, scale=gc_err_sd(gc_meas_means), random_state=rs)
        ...
        if i>=3:
            qpcr_meas_means = np.column_stack([y[:, 3]]*3)
            qpcr_meas = st.norm.rvs(loc=qpcr_meas_means, scale=qpcr_err_sd(qpcr_meas_means), random_state=rs)
...
def gen_flat_model_and_data(theta, seed):
    init_dce_list = [0.01, 0.05, 0.20, 0.35, 0.50]
    t95_list = list()
    for init_dce in init_dce_list:
        t95_list.append(calc_t95(theta, init_dce))
    gc_meas, qpcr_meas, gc_stdev, qpcr_stdev = gen_syn_data(theta, t95_list, seed=seed)
    ...
    def mrd_model(x, theta):
        ...
        gc_mod =  np.stack([np.column_stack([y[1:,:3]]*3) for y in y_list])
        gc_mod_flat = gc_mod.flatten()
        qpcr_mod = np.stack([np.column_stack([y[1:, 3]]*3) for y in y_list[3:]])
        qpcr_mod_flat = qpcr_mod.flatten()
...
def analyze_regression(model, x_data, y_data, meas_stdev, theta_hat, theta_true, normalize=True, Q_file='none', conf_level=0.90):
    ...
    marg_stdev = np.sqrt(np.diag(A_inv))
    lo_marg_thres = st.norm.ppf(alpha/2, loc=theta_hat_mant, scale=marg_stdev)
    hi_marg_thres = st.norm.ppf(1-alpha/2, loc=theta_hat_mant, scale=marg_stdev)
    in_marg_ci = np.logical_and(lo_marg_thres<theta_true_mant, theta_true_mant<hi_marg_thres)
    rmoe = (hi_marg_thres - lo_marg_thres)/theta_hat_mant

    return Q, in_exact_cr, in_approx_cr, in_marg_ci, rmoe
...
def load_theta_hats(theta):
    ...
    fit_res_arr = np.load('fit_res/'+str(seed)+'.npy')
    return fit_res_arr[-5:]*scale


def perform_regress_analyses(fit_res_file):
    theta = np.load(fit_res_file)[0]
    try:
        seed = calc_seed(theta)
        theta_hats = load_theta_hats(theta)
        in_cr = np.zeros((theta_hats.shape[0],2))
        in_ci = np.zeros((theta_hats.shape[0],8))
        all_rmoe = np.zeros((theta_hats.shape[0],8))
        for i, theta_hat in enumerate(theta_hats):
            Q_file = 'Q/'+str(seed)+'_'+str(i)+'Q.npy'
            mrd_model, x_data, y_data, meas_stdev= gen_flat_model_and_data(theta, seed=seed+i)
            if exists(Q_file):
                Q, in_exact_cr, in_approx_cr, in_marg_ci, rmoe  = analyze_regression(mrd_model, x_data, y_data, meas_stdev, theta_hat, theta, Q_file=Q_file)
            else:
                Q, in_exact_cr, in_approx_cr, in_marg_ci, rmoe  = analyze_regression(mrd_model, x_data, y_data, meas_stdev, theta_hat, theta)
                np.save(Q_file, Q)

            in_cr[i] = np.array([in_exact_cr, in_approx_cr])
            in_ci[i] = in_marg_ci
            all_rmoe[i] = rmoe
        np.save('in_cr/'+str(seed), in_cr)
        np.save('in_ci/'+str(seed), in_ci)
        np.save('rmoe/'+str(seed), all_rmoe)
    except:
        pass
...
if __name__ == "__main__":
    ...
    pool.map(perform_regress_analyses, fit_res_files)
    pool.terminate()

    # for fit_res_file in tqdm(fit_res_files):
    #     theta = np.load(fit_res_file)[0]
    #     perform_regress_analyses(theta)
