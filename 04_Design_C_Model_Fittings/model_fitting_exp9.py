...
def swsr(theta, t95_list, gc_meas, qpcr_meas):
    init_dce_list = [0.01, 0.05, 0.20, 0.35, 0.50]
    y_list = []
    gc_mod_list = []
    qpcr_mod_list = []
    for i, init_dce in enumerate(init_dce_list):
        ...
        y = np.clip(ivp_soln[0], 1e-6, None)
        gc_mod_list.append(np.column_stack([y[:,:3]]*3))
        if i>=3:
            qpcr_mod_list.append(np.column_stack([y[:, 3]]*3))

    gc_resid = gc_meas - np.stack(gc_mod_list)
    qpcr_resid = qpcr_meas - np.stack(qpcr_mod_list)
...
def run_trial(theta):
    ...
    x_hat_list = list()
    for i in range(5):
        gc_meas, qpcr_meas = gen_syn_data(theta, t95_list, seed=seed+i)

        def obj_fun(x):
            try:
                res = swsr(x*scale, t95_list, gc_meas, qpcr_meas)
            except:
                res = 1e12
            return res

        de_bounds = np.column_stack((x_true/10, x_true*10))
        ...
        x_hat_list.append(de_res.x)

    filename = 'fit_res/'+str(seed).zfill(8)
    np.save(filename, np.stack([theta, x_true]+x_hat_list))
...
def pick_samples(node):
    res_list = list()
    chains = [load_samples('alpha_hmc_samp.npy'), load_samples('beta_hmc_samp.npy'), load_samples('gamma_hmc_samp.npy')]
    chain_lengths = [len(chain) for chain in chains]
    max_idx_even = 5*(min(chain_lengths)//5)
    chain_arr_even = np.stack([chain[:max_idx_even] for chain in chains])
    res_list.append(np.reshape(chain_arr_even, (5,-1,8), order='F')[node])
    chain_remainders = [chain[max_idx_even:] for chain in chains]
    chain_arr_uneven = np.row_stack(chain_remainders)
    max_idx_uneven = 5*(chain_arr_uneven.shape[0]//5)
    res_list.append(np.reshape(chain_arr_uneven[:max_idx_uneven], (5,-1,8), order='F')[node])
    return np.row_stack(res_list)


if __name__ == "__main__":
    node_idx = 0
    samples = pick_samples(node_idx)
    # run_trial(samples[1003])
