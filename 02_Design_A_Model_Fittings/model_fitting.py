...
def swsr(theta, t95, gc_meas, qpcr_meas):
    t_samp = np.linspace(0, t95, 21)
    Qa = 0.021/t95
    y0 = np.array([0.15, 0.0, 0.0, 1E-1, 0.125, 0.035])
    ivp_soln = odeint(
...
def run_trial(theta):
    ...
    if exists('fit_res/'+str(seed).zfill(8)+'.npy'):
        return None
    ...
    x_hat_list = list()
    for i in range(5):
        gc_meas, qpcr_meas = gen_syn_data(theta, t95, seed=seed+i)

        def obj_fun(x):
            try:
                res = swsr(x*scale, t95, gc_meas, qpcr_meas)
            except:
                res = 1e12
            return res
